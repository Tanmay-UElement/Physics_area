...
export default function MomentumConservationPage() {
  ...
  const [userVelocity, setUserVelocity] = useState<number>(9.0);
  ...
  const handleLaunch = (vFinal: number) => {
    setUserVelocity(vFinal);
  ...
  if (isLoading || !currentRound) {
    return <LoadingOverlay conceptName="Momentum Conservation (Unequal)" />;
  }

  const momentumRound = currentRound as MomentumRoundData;
