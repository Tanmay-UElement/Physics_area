...
import { VoltCapacitorCanvas } from '@/components/match/VoltCapacitorCanvas';
import { VoltCapacitorSolvePanel } from '@/components/match/VoltCapacitorSolvePanel';
...
export default function VoltCapacitorRacePage() {
  ...
  const [userTime, setUserTime] = useState<number>(1.00);
  const [currentResult, setCurrentResult] = useState<RoundResult | null>(null);

  const isLight = theme === 'light';

  useEffect(() => {
    startMatch('volt-capacitor-race');
  ...
  if (isLoading || !currentRound) {
    return <LoadingOverlay conceptName="Capacitor Race (RC Charging)" />;
  }

  const capacitorRound = currentRound as VoltCapacitorRoundData;
