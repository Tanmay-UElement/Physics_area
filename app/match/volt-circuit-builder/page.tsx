...
import { VoltCircuitCanvas } from '@/components/match/VoltCircuitCanvas';
import { VoltCircuitSolvePanel } from '@/components/match/VoltCircuitSolvePanel';
...
export default function VoltCircuitBuilderPage() {
  ...
  const [userResistance, setUserResistance] = useState<number>(6.0);
  const [currentResult, setCurrentResult] = useState<RoundResult | null>(null);

  const isLight = theme === 'light';

  useEffect(() => {
    startMatch('volt-circuit-builder');
  ...
  const handleLaunch = (resistance: number) => {
    setUserResistance(resistance);
  ...
  if (isLoading || !currentRound) {
    return <LoadingOverlay conceptName="Circuit Builder (Ohm's Law)" />;
  }

  const voltRound = currentRound as VoltCircuitRoundData;
